...
def csv_to_xlsx(
    ...
    with csv_path.open("r", encoding="utf-8") as csv_file:
        ...
        ws.append(
            reader.fieldnames
        )  # Добавляем заголовки как первую строку в Excel лист

        for row in reader:
            row_values = (
                []
            )  # Создаем пустой список для хранения всех значений из текущей строки CSV
            for (
                field
            ) in (
                reader.fieldnames
            ):  # Получаем значение из текущей строки для конкретной колонки
                value = row[field]
                row_values.append(value)
            ws.append(
                row_values
            )  # Добавляем значения текущей строки как новую строку в Excel лист

    xlsx_path = Path(xlsx_path)  # Преобразуем выходной путь Excel в объект Path
    wb.save(
        xlsx_path
    )  # Сохраняем созданную книгу, все данные, которые мы добавили, записываются в файл
