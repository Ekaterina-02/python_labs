...
def tokenize(text: str) -> list[str]:
    text_new = (
        []
    )  # пустой список для хранения обработанных символов, работаем только с дефисами
    for i in range(len(text)):
        if text[i] == "-":
            if (
                i > 0 and i < len(text) - 1
            ):  # дефис не находится в начале или конце строки
                if (text[i - 1].isalnum() or text[i - 1] == "_") and (
                    text[i + 1].isalnum() or text[i + 1] == "_"
                ):  # символы слева и справа от дефиса-буквы/цифры или подчеркивания
                    text_new.append("_")  # временная замена дефиса
        else:
            text_new.append(text[i])
    text_new = (
        "".join(text_new) + " "
    )  # преобразуем спиосок символов в строку и пробел, без него слово не добавится в результат
...
def count_freq(tokens: list[str]) -> dict[str, int]:
    ...
    for i in tokens:
        result[i] = (
            result.get(i, 0) + 1
        )  # ищет слово i в словаре result, если есть-возвращает количество, если нет-возвращает 0
    ...
    for key in s:
        sorted_dict[key] = result[
            key
        ]  # сохраняем значение из исходного словаря в новый, под тем же ключом
    return sorted_dict
...
def top_n(freq: dict[str, int], n: int = 5) -> list[tuple[str, int]]:
    ...
    sorted_letters = sorted(
        spisoc, key=lambda x: x[0]
    )  # cортируем по алфавиту (по слову)
    sorted_counts = sorted(
        sorted_letters, key=lambda x: x[1], reverse=True
    )  # cортируем список по количеству
